...
public class Practice {
  ...
  private <T> void printVertexVals(Vertex<T> vertex, HashSet seen) {
    if (vertex == null) return;
  ...
  public <T> Set<Vertex<T>> reachable(Vertex<T> vertex, HashSet<Vertex<T>> output) {
    if (vertex == null) return new HashSet<>();
    if (output.contains(vertex)) return output;

    output.add(vertex);

    if (vertex.neighbors == null) return output;
    for (Vertex<T> neighbor : vertex.neighbors) {
      Set<Vertex<T>> neighborSet = reachable(neighbor, output);

      for (Vertex<T> child : neighborSet) {
        output.add(child);
      }
    }
    return output;
  ...
  private int max(Vertex<Integer> vertex, HashSet<Vertex<Integer>> seen) {
    if (vertex == null) return Integer.MIN_VALUE;
    if (seen.contains(vertex)) return Integer.MIN_VALUE;

    seen.add(vertex);
    int output = vertex.data;

    for (Vertex<Integer> neighbor : vertex.neighbors) {
      int challenger = max(neighbor, seen);
      if (challenger > output) output = challenger;
    }
    return output;
  }
  ...
  public <T> Set<Vertex<T>> leaves(Vertex<T> vertex, HashSet<Vertex<T>> seen) {
    if (vertex == null) return new HashSet<>();
    if (seen.contains(vertex)) return new HashSet<>();
    HashSet<Vertex<T>> output = new HashSet<>();
    seen.add(vertex);
    if (vertex.neighbors == null || vertex.neighbors.isEmpty()) {
      output.add(vertex);
      return output; }
    for (Vertex<T> neighbor : vertex.neighbors) {
      Set<Vertex<T>> neighborSet = leaves(neighbor, seen);
  ...
  private boolean hasStrictlyIncreasingPath(Vertex<Integer> start, Vertex<Integer> end, HashSet<Vertex<Integer>> seen) {
    if (start == null) return false;
    if (start == end) return true;
    seen.add(start);
    if (start.neighbors == null || start.neighbors.isEmpty()) return false;
